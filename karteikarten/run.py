...
def start(request, set_id):
  ...
  if 'box' in request.GET:
    ...
    for cardbox in cardboxes:
      ...
      #If time between now and last review time is more than 6 hours before the
      #review frequency
      if ((diff.days * 24) + (diff.seconds / 3600) >=
          (cardbox.review_frequency * 24) - 6):
        cardboxes_to_review.append({
            'box' : cardbox.pk,
            'card_count' : cardbox.card_set.count()
          }) #Review this cardbox

  request.session['cardset'] = set_id #ID of cardset that will be reviewed
  request.session['cardboxes'] = cardboxes_to_review #IDs of cardboxes that will
                                                     #be reviewed
  request.session['total_cards'] = total_cards #Total cards to review; Not used
                                               #in normal review model
  request.session['cur_cardbox_index'] = 0 #Index into array, representing the
                                           #ID of the current cardbox
  request.session['cards_reviewed'] = [] #IDs of cards already reviewed
  request.session['cards_reviewed_this_set'] = [] #IDs of cards already reviewed
                                                  #in the current set

  if 'box' in request.GET:
    return HttpResponseRedirect(reverse('run-run') + '?box=' +
                                request.GET['box'])
...
def run(request):
  ...
  try:
    ...
    return render_to_response('error.html', {
        'message' : _('review-session-expired'),
        'go_back_to' : reverse('centre'),
        'title' : _('error'),
        'site_link_chain' : zip([], [])
      }, context_instance = RequestContext(request))

  cur_cardbox = get_object_or_404(CardBox,
                                  pk = cardboxes[cur_cardbox_index]['box'])
  ...
  for card in cards:
    if card.pk not in cards_reviewed:
      ...
      return render_to_response('run/review.html', {
          'card' : card,
          'card_box' : cur_cardbox,
          'card_set' : cur_cardbox.parent_card_set,
          'study_mode' : _('study-type-normal'),
          'num_completed_cards' : len(cards_reviewed_this_set),
          'num_cards' : cardboxes[cur_cardbox_index]['card_count'],
          'url_append' : ''
        }, context_instance = RequestContext(request))
...
def runSpecificBox(request):
  ...
  if 'box' in request.GET:
    try:
      ...
      current_box = get_object_or_404(CardBox, pk = current_box_id,
                                      owner = request.user)
    ...
    return render_to_response('error.html', {
        ...
        'message' : _('no-card-box-specified'),
        'go_back_to' : reverse('centre'),
        'title' : _('error'),
        'site_link_chain' : zip([], [])
      }, context_instance = RequestContext(request))
  ...
  for card in cards:
    if card.pk not in cards_reviewed:
      ...
      return render_to_response('run/review.html', {
          'card' : card,
          'card_box' : current_box,
          'card_set' : cardset,
          'study_mode' : study_mode,
          'num_completed_cards' : len(cards_reviewed),
...
def correct(request, card_id):
  ...
  if 'box' not in request.GET: #Only update card status if not reviewing a
                               #specific box
    card = get_object_or_404(Card, pk = card_id, owner = request.user)
    card_set = card.parent_card_set
    cardboxes = CardBox.objects.filter(owner = request.user,
                                       parent_card_set = card_set). \
                order_by('review_frequency')
    to_cardbox = None
...
def incorrect(request, card_id):
  ...
  #Only update card status if not reviewing a specific box or if reviewing cards
  #from no box
  if 'box' not in request.GET or request.GET['box'] == 'None':
    card = get_object_or_404(Card, pk = card_id, owner = request.user)
    card_set = card.parent_card_set
    cardboxes = CardBox.objects.filter(owner = request.user,
                                       parent_card_set = card_set). \
    ...
    if not len(cardboxes):
      return render_to_response('error.html', {
          'message' : _('cannot-move-to-first-box'),
          'go_back_to' : reverse('run-run') + url_append,
          'title' : _('error'),
          'site_link_chain' : zip([], [])
        }, context_instance = RequestContext(request))

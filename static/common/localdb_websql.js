...
{
  function LocalDatabaseWebSQL(database_name, verison, database_longname, size,
                               success_callback, error_callback)
  {
    if (typeof(openDatabase) == 'function')
    {
      this.websql_db = openDatabase(database_name, '1.0', database_longname,
                                    size);
      var migrations_system = new MigrationsSystemWebSQL(this.websql_db,
        function()
        {
  ...
  @param error_callback Function to call following an error. Takes two
    ...
    message.
  */
  LocalDatabaseWebSQL.prototype.Select = function(table, attributes, start, end,
    success_callback, error_callback)
  {
    ...
    for (attribute in attributes)
    {
      ...
      if (SCHEMA_WEBSQL[table][attribute] == 'string')
        ...
        sql += attribute + '=' +
               Math.round(attributes[attribute].getTime() / 1000) + ' AND ';
      else
        sql += attribute + '=' + attributes[attribute] + ' AND ';
    }
    sql = sql.substring(0, num_attributes > 0 ? sql.length - 5 : sql.length) +
                        (start !== null &&
                         end !== null ? ' LIMIT ' + start + ', ' + end : '') +
                        ';';
  ...
  @param error_callback Function to call following an error. Takes two
    ...
    message.
  */
  LocalDatabaseWebSQL.prototype.Update = function(table, id, attributes,
    success_callback, error_callback)
  {
    this.websql_db.transaction(function(transaction)
      {
        ...
        for (attribute in attributes)
        {
          ...
          if (SCHEMA_WEBSQL[table][attribute] == 'string')
            ...
            sql += attribute + '=' +
                   Math.round(attributes[attribute].getTime() / 1000) + ',';
          else
            sql += attribute + '=' + attributes[attribute] + ',';
        }
  ...
  @param attributes Object of attribute/value pairs to modify.
  @param success_callback Function to call following the request if successful.
    Takes one parameter: the ID of the inserted item.
  @param error_callback Function to call following an error. Takes two
    parameters: the error type (Possible values: 'local-db', 'no-attributes')
    and the error message.
  */
  LocalDatabaseWebSQL.prototype.Insert = function(table, attributes,
    success_callback, error_callback)
  {
    this.websql_db.transaction(function(transaction)
    {
      ...
      sql = 'INSERT INTO ' + table + ' (' +
            fields.substr(0, fields.length - 1) + ') VALUES (' +
  ...
  @param success_callback Function to call if successful. Takes 1 parameter: The
    max value.
  @param error_calllback Function to call if an error occurs. Takes 2
    parameters: The error type (Possible values: 'local-db') and the error
    message.
  */
  LocalDatabaseWebSQL.prototype.Max = function(table, field, success_callback,
                                               error_callback)
  {
    this.websql_db.transaction(function(transaction)
      {
        transaction.executeSql('SELECT MAX(' + field + ') AS max FROM ' +
          table + ';', [], function(transaction, results)
          {
  ...
  @param error_callback Function to call if an error occurs. Takes 2 parameters:
    The error type (Possible values: 'local-db') and the error message.
  */
  LocalDatabaseWebSQL.prototype.Count = function(table, attributes,
    success_callback, error_callback)
  {
    var where = '';
    for (attribute in attributes)
    ...
    this.websql_db.transaction(function(transaction)
      {
        transaction.executeSql('SELECT COUNT(*) AS count FROM ' + table +
